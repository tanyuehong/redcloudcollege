...
public class MemuHelper {
     ...
     * @return
     */
    public static List<JSONObject> bulid(List<EduPermission> treeNodes) {
        List<JSONObject> meuns = new ArrayList<>();
        if(treeNodes.size() == 1) {
            EduPermission topNode = treeNodes.get(0);
            //左侧一级菜单
            List<EduPermission> oneMeunList = topNode.getChildren();
            for(EduPermission one :oneMeunList) {
                ...
                List<EduPermission> twoMeunList = one.getChildren();
                for(EduPermission two :twoMeunList) {
                    ...
                    List<EduPermission> threeMeunList = two.getChildren();
                    for(EduPermission three :threeMeunList) {
